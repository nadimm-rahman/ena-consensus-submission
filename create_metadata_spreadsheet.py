...
# IMPROVEMENTS:
#   --> Add to generate_chromlistfile.sh to include a list of chromosome list files.
...
def get_args():
    ...
    parser.add_argument('-h', '--headers', type=str, help='Consensus/assembled sequence headers to be included as assembly names. Format: List of consensus/assembled sequence headers - one per line', required=True)
    parser.add_argument('-c', '--chromosome_list', type=str, help='Chromosome list files created to be used in submission. Format: List of names of chromosome list files - one per line', required=True)
    parser.add_argument('-f', '--fasta_files', type=str, help='Fasta files of assembled/consensus sequence to be submitted. Format: List of names of fasta files - one per line', required=True)
...
class MetadataFromDatabase:
    ...
    def setup_connection(self):
        ...
        try:
            dsn = cx_Oracle.makedsn("ora-vm-009.ebi.ac.uk", 1541, service_name="ERAPRO")        # Try connection to ERAPRO with credentials
    ...
    def fetch_metadata(self):
        ...
        if self.connection is not None:
            ...
            df.columns = ['project_accession', 'sample_accession', 'run_accession']      # Add column headers to the dataframe
            return df
...
if __name__ == '__main__':
    ...
    assembly_names = pd.read_csv(args.headers, sep="\t", header=None)
    ...
    # STEP 1 --> Create columns for each dataframe to enable for merging with main dataframe

    # Create column for the fasta file defining the corresponding run
    # linked_fasta_files = pd.DataFrame(columns=['FASTA', 'run_accession'])
    # for index, row in fasta_files.iterrows():
    #     fasta = row[0]
    #     run = fasta.split('.')[0]
    #     df = pd.DataFrame([[fasta, run]], columns=['FASTA', 'run_accession'])
    #     linked_fasta_files = linked_fasta_files.append(df, ignore_index=True)
    ...
    # linked_assembly_names = pd.DataFrame(columns=['ASSEMBLYNAME', 'run_accession'])
    # for index, row in assembly_names.iterrows():
    #     assembly = row[0]
    #     run = assembly.split('_')[0]
    #     df = pd.DataFrame([[assembly, run]], columns=['ASSEMBLYNAME', 'run_accession'])
    #     linked_assembly_names = linked_assembly_names.append(df, ignore_index=True)
    ...
    # linked_chromosome_lists = pd.DataFrame(columns=['CHROMOSOME_LIST', 'run_accession'])
    # for index, row in chromosome_lists.iterrows():
    #     chromosome = row[0]
    #     run = chromosome.split('_')[0]
    #     df = pd.DataFrame([[chromosome, run]], columns=['CHROMOSOME_LIST', 'run_accession'])
    #     linked_chromosome_lists = linked_chromosome_lists.append(df, ignore_index=True)
    ...
    set_parameters = pd.DataFrame(parameter_values, columns=['ASSEMBLY_TYPE', 'COVERAGE', 'PROGRAM', 'PLATFORM', 'MINGAPLENGTH', 'MOLECULETYPE'])


    # STEP 3 --> Merge dataframes together
    project_assembly = pd.merge(project_data, linked_assembly_names, on='run_accession')        # Include the assembly name information
    project_assembly = pd.concat([project_assembly, set_parameters], axis=1, sort=False)        # Include all the other set columns
    project_assembly_fasta = pd.merge(project_assembly, linked_fasta_files, on='run_accession')     # Include the names of fasta files
    total_metadata = pd.merge(project_assembly_fasta, linked_chromosome_lists, on='run_accession')      # Include the names of chromosome list files

    total_metadata = total_metadata.rename(columns={"study_accession": "STUDY", "sample_accession": "SAMPLE", "run_accession": "RUN_REF"})      # Change column names to match the manifest file headers
    metadata_prefix = args.project_metadata.split('.')[0]     # Get the first part of the name
    metadata_filename = metadata_prefix+"_Consensus_Metadata.txt"
